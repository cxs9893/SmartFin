...
- 功能 5：容器本地 LLM 依赖补齐
  - `pyproject.toml` 新增可选依赖组 `local-llm`（`torch`、`transformers`）。
  - Dockerfile 安装升级为包含 `.[embedding,local-llm]`，修复容器内 `No module named 'torch'`。
...
7. `docker compose build --no-cache`（exit 0）
8. `docker compose up -d`（exit 0，清理冲突容器后）
